...
const PatientsList = () => {
  const {patients, setPatientsData, patientInitialValue, addPatient, editPatient, setpatientInitialValue} = useGlobalStore();
  ...
  return (
    <>
      <PatientModal isOpen={open} initialValues={patientInitialValue} onClose={setOpen} onSubmit={onSubmit}/>
      <div className="flex justify-between mt-5 mr-2">
        <div>
          <h1 class="font-bold text-xl m-2">List of patients</h1>
        </div>
        <div>
          <button className="bg-blue-400 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded" onClick={()=> {
            setOpen(true)
            setpatientInitialValue({name: "", description: "", website: ""})
          }
            }>
            Add Patient
          </button>
